'use client'
import { UserCreateAccount, UserProfileInterface } from "@/app/action"
import Link from "next/link"
import { useRouter } from "next/navigation"
import { useActionState, useEffect } from "react"

export const UpdateUserComponent = ({ data }: { data: UserProfileInterface }) => {
    ...
    return (
        <div className="flex justify-center items-center min-h-screen flex-col w-full">
            <form action={action} className="flex flex-col justify-center gap-4 w-2/5 max-md:w-full h-1/2 p-2">
                <h1 className="text-2xl mb-4">Alterar conta</h1>
            ...
            <Link href="/user" className="w-full text-center text-blue-500 py-2 rounded-md hover:text-blue-600 hover:underline transition-all duration-300">
                Início
            </Link>
        </div>
    )
